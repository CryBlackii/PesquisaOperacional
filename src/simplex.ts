...
export function faseI(
    ...
    const colunasParaBasica: number[] = []; // Índices das variáveis na base inicial.
    
    // Montar a Base Inicial para a Fase I
   let slackSurplusIndex = 0;

    for (let i = 0; i < m; i++) {
    if (constraintTypes[i] === '<=') { // era aqui um dos erros tbm****
        // excesso +1
        colunasParaBasica.push(numDecisionVars + slackSurplusIndex);
        slackSurplusIndex++;
    } else if (constraintTypes[i] === '>=') {
        // excesso -1
        matrizComArtificiais[i][numDecisionVars + slackSurplusIndex] = -1; // Variável de excesso
        matrizComArtificiais[i][n + currentArtificialIndex] = 1; // Variável artificial
        colunasParaBasica.push(n + currentArtificialIndex); // Base = artificial
        currentArtificialIndex++;
        slackSurplusIndex++; // Incrementa apenas para rastrear a coluna de excesso
    } else { // '='
        // excesso =
    ...
    colunasParaBasica.forEach((basicVarIndex, i) => {
        ...
        if (type === '>=' || type === '=') {
             if(basicVarIndex >= n) expressaoArtificial[basicVarIndex] = 1;
        }
    });
    ...
    // arruma a função objetivo artificial, ou seja arruma para o custos relativos das variaveis básicas devem ser 0.
    for (let i = 0; i < m; i++) {
        const basicVarIndex = colunasParaBasica[i];
        if (custoArtificial[basicVarIndex] === 1) {
    ...
    // Chama a Fase II para resolver o problema artificial
    ...
    // Se o resultado é nulo ou o custo ótimo for 0 > 1e-6 (Tolerância numérica para evitar falsos positivos), o problema original é inviável.
    if (resultadoFase1 === null || resultadoFase1[0] > 1e-6) {
        return null;
    }
    
    // Verifica se alguma variável artificial permaneceu na base com valor não nulo
    const artificialNaBase = colunasParaBasica.some(varIndex => varIndex >= n);
    if (artificialNaBase) {
        return null; // trata casos degenerados como inviáveis.
    }
...
export function faseII(
    ...
    matrizBasica: number[][], // Matriz B
    colunasParaBasica: number[],
    matrizNaoBasica: number[][], // Matriz N
    colunasParaNaoBasica: number[],
    valoresDesigualdade: number[], // Vetor b
    vetorExpressaoPrincipal: number[], // Vetor c
    tipoOtimizacao: string,
    iteracao: number = 1
): [number, number[], number] | null {
    ...
    if (menorCusto >= -1e-9) { // Usa uma tolerância para comparação com zero.
        let valorOtimo = 0;
        const CbB = multiplicaMatriz(custoBasico, xBasico); // Z = cB * xB
        valorOtimo = CbB[0][0];
    ...
    //Determinar a variável que entra na base, a variável que entra é a que tem o custo relativo mais negativo pra ser mais rapidex
    const indiceVariavelEntrada = custoRelativo.indexOf(menorCusto);

    // Determinar a variável que sai da base
    ...
    // aqui vai atualizar a base, com isso a variavel que entra (das não basicas) troca de lugar com a variável que sai
